document.body.onload = function() {
  ...
  function GetURLParameter (sParam) {
    ...
    for (var i = 0; i < sURLVariables.length; i++) {
      ...
      if (sParameterName[0] == sParam && sParameterName[1]) return sParameterName[1].replace(/"/g, '');
    }
  }
  ...
  function initiate() {
    ...
    BizApp.login({
      ...
      if (!userData.err) {
        ...
        $('#failed-page').removeClass('d-none');
        $('#failed-page #error-statement').html(userData.err);
      }
    });
    ...
    if (error) {
      ...
      $('#failed-page').removeClass('d-none');
      $('#failed-page #error-statement').html(error);
    }
    else if (message) {
      $('#loading').addClass('d-none');
      $('#failed-page').removeClass('d-none');
      $('#failed-page #error-statement').html(message); 
    }
    else if (success) {
      $('#loading').addClass('d-none');
      $('#success-page').removeClass('d-none');
      $('#success-email').html(email);
      if (!email) $('#check-email').addClass('d-none');
    }
    else {
      $('#loading').addClass('d-none');
